...
def benders_decomposition(n: int, m0: int, total_r, A_sparse, B_sparse, config: BendersConfig | None = None, problem_type: str = "general") -> Tuple[float, np.ndarray, List, Dict]:
    if config is None:
    ...
    for iteration in range(config.max_iterations_per_phase):
        r_vars, theta = solve_master_problem(blocks_metadata, m0, total_r, all_cuts, config)
        args_list = []
        for block_id, start, end in blocks_metadata:
            r_i = r_vars[0]
            args_list.append((block_id, start, end, A_meta, B_meta, d, x_prev, r_i, config.__dict__))
        ...
        for block_id, obj, x_block, *_ in results:
            for bid, s, e in blocks_metadata:
                if bid == block_id:
                    x_prev[s:e] = x_block
                    break
        dual_gaps = {bid: theta[idx] - obj for idx, (bid, _, _) in enumerate(blocks_metadata)}
        blocks_metadata = repartition_blocks(blocks_metadata, dual_gaps, n)

    cleanup_shared_memory()
    return float(np.sum(x_prev)), x_prev, all_cuts, {}
